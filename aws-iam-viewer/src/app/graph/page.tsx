...
import { analyzeAllPolicies, PolicyAnalysisResult } from '@/lib/policy-analysis';
...
export default function GraphPage() {
  ...
  const buildGraphData = useCallback((iamData: ProcessedIAMData, filters: string[] = [], securityAnalysisData: PolicyAnalysisResult[] = []) => {
    const newNodes: Node[] = [];
    const newEdges: Edge[] = [];
    ...
    rolePolicies.forEach((policy, index) => {
      ...
      const securityData = securityAnalysisData.find(analysis => analysis.policyId === policy.PolicyId);
      const isHighRisk = securityData?.isHighRisk || false;
      const hasFlags = securityData?.flags.length || 0;
      ...
      newNodes.push({
        ...
        style: {
          background: isHighRisk ? '#FEE2E2' : nodeTypes.policy.bgColor,
          border: `2px solid ${isHighRisk ? '#DC2626' : nodeTypes.policy.color}`,
          borderRadius: '4px',
          width: 350,
          height: 60,
          fontSize: '12px',
  ...
  useEffect(() => {
    const loadCurrentUpload = async () => {
      try {
        ...
        // Build graph data
        const { nodes: graphNodes, edges: graphEdges } = buildGraphData(uploadResult.data, selectedFilters, analysis);
        setNodes(graphNodes);
        setEdges(graphEdges);
        setIsLoading(false);
      } catch (error) {
        ...
        <CardContent>
          <div className="space-y-4">
            ...
            <div className="border-t pt-4">
              <h4 className="text-sm font-medium mb-2">Security Indicators</h4>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="flex items-center space-x-2">
                  <div className="w-4 h-4 rounded border-2 bg-red-50 border-red-600 relative">
                    <AlertTriangle className="h-2 w-2 text-red-600 absolute -top-1 -right-1" />
                  </div>
                  <span className="text-sm">High Risk Policy</span>
                </div>
                <div className="flex items-center space-x-2">
                  <div className="w-4 h-4 rounded border-2 bg-yellow-50 border-yellow-600 relative">
                    <div className="h-1 w-1 bg-yellow-500 rounded-full absolute -top-0.5 -right-0.5"></div>
                  </div>
                  <span className="text-sm">Policy with Security Flags</span>
                </div>
                <div className="flex items-center space-x-2">
                  <div className="w-4 h-4 rounded border-2 bg-green-50 border-green-600"></div>
                  <span className="text-sm">Secure Policy</span>
                </div>
              </div>
            </div>
          </div>
