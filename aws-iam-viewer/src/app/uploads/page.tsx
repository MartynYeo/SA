...
export default function UploadsPage() {
  ...
  useEffect(() => {
    const loadUploads = async () => {
      try {
        setIsLoading(true);
        const uploadsResult = await apiService.getAllUploads();
        if (uploadsResult.error) {
          console.error('Failed to load uploads:', uploadsResult.error);
          return;
        }
        ...
        const currentIdResult = await apiService.getCurrentUploadId();
        setCurrentUploadId(currentIdResult.uploadId || null);
      } catch (error) {
  ...
  const handleSwitchUpload = async (uploadId: string) => {
    try {
      const result = await apiService.setCurrentUploadId(uploadId);
      if (result.success) {
        setCurrentUploadId(uploadId);
        router.push('/dashboard');
      } else {
        console.error('Failed to switch upload:', result.error);
      }
    } catch (error) {
    ...
    try {
      ...
      if (result.success) {
        ...
        if (currentUploadId === uploadId) {
          await apiService.setCurrentUploadId(null);
          setCurrentUploadId(null);
        }
      } else {
        console.error('Failed to delete upload:', result.error);
      }
    } catch (error) {
            ...
            <TableBody>
              {sortedUploads.map(([uploadId, upload]) => (
                <TableRow key={uploadId}>
                  ...
                  <TableCell>{formatDateTime(upload.uploaded_at)}</TableCell>
